...
# System prompts
TURKISH_PROMPT = """Sen çok gelişmiş, empatik ve insan gibi davranan bir AI asistanısın.
...
ENGLISH_PROMPT = """You are a highly advanced, empathetic, and human-like AI assistant.
...
def get_prompt(message: str, user_name: Optional[str] = None) -> str:
    ...
    if user_name:
        if "tr" in base_prompt.lower():
            base_prompt += f"\n\nKullanıcının adı: {user_name}. Ona ismiyle hitap et."
        else:
            base_prompt += f"\n\nUser's name is: {user_name}. Address them by name."
...
def chat_endpoint(req: ChatRequest):
    try:
        ...
        try:
            ...
            traceback.print_exc()
            ai_response = "I'm having trouble right now. Please try again later."
...
@app.get("/")
def read_root():
    return {"message": "Thesis Chatbot API", "status": "running"}
